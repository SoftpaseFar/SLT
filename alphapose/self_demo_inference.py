...
def load_model():
    ...
    return pose_model, tracker, pose_dataset, detector


def process(model, tracker, pose_dataset, detector):
    ...
    if mode == 'webcam':
        det_loader = WebCamDetectionLoader(input_source, detector, cfg, args)
        det_worker = det_loader.start()
    elif mode == 'detfile':
        det_loader = FileDetectionLoader(input_source, cfg, args)
        det_worker = det_loader.start()
    else:
        det_loader = DetectionLoader(input_source, detector, cfg, args, batchSize=args.detbatch, mode=mode,
